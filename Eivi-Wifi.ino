...
String target_ssid = "Not_Set"; 
const char* discord_webhook = "YOUR_DISCORD_WEBHOOK_URL";
...
</head><body><div class="h"><h1>Control Panel</h1></div><div class="c">
<div class="card"><h3>Current Target:</h3><b style="color:#4acbd6" id="curr">...</b></div>
<a href="/scan" class="btn" style="background:#4acbd6">Scan Networks</a>
<div id="list" class="card" style="margin-top:20px;text-align:center">Network list will appear here after scan...</div>
...
void updateWiFi() {
  ...
  if (wifiWantOn) WiFi.softAP("Admin_Panel", "123456789");
...
void setup() {
  ...
  webServer.on("/scan", []() {
    ...
    String h = "<h3>Networks Found:</h3>";
    for(int i=0; i<n; i++) h += "<div class='net-item'><a href='/set?s="+WiFi.SSID(i)+"'>"+WiFi.SSID(i)+"</a><span>"+String(WiFi.RSSI(i))+"dBm</span></div>";
    webServer.send(200, "text/html", h + "<br><a href='/'>Back</a>");
  });

  webServer.on("/set", []() {
    target_ssid = webServer.arg("s");
    webServer.send(200, "text/html", "<h2>Target Set: "+target_ssid+"</h2><a href='/'>Back to Panel</a>");
  });
